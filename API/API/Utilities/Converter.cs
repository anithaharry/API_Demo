...
namespace API.Utilities
{
    public class Converter
    {
        ...
        private static string NumberToWords(long number, string scaleValue = "")
        {
            ...
            else
            {
                ...
                //extract the first part of the number which gives you the scale and the single number that can be recursed to find the worf format 
                long extractedfirstPart = number / unitValue;

                //get the remainder of the number into the second part and evaluate the second part as well 
                long extractedSecondPart = number % unitValue;
                finalString = string.Format("{0} {1}", NumberToWords(extractedfirstPart, unitValueString), NumberToWords(extractedSecondPart)).Trim();
            }


            return string.Format("{0} {1}", finalString, scaleValue).Trim();
        }
    }
}